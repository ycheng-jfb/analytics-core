...
SET max_date = (SELECT coalesce(MAX(date),'2022-01-01'::date)
FROM reporting_prod.sxf.retail_drive_time);
...
WHERE ds.store_brand_abbr = 'SX'
    ...
    AND store_sub_type not in ('Browser')
);
...
LEFT JOIN LAKE_CONSOLIDATED_VIEW.ULTRA_MERCHANT.STORE  AS  v ON v.store_id = st.store_id
WHERE
...
LEFT JOIN reporting_base_prod.data_science.sxf_retail_driving_distance rdd ON TO_CHAR(rdd.vip_zip) = TO_CHAR
...
;
select * from reporting_prod.sxf.retail_drive_time;
DELETE FROM reporting_prod.sxf.retail_drive_time
WHERE date >= DATE_TRUNC('year',$max_date);


INSERT INTO reporting_prod.sxf.retail_drive_time (grain,sheet,date,store_name,region,country,district,driving_time,distance,shopped_in_retail,is_bop_vip,is_activated,distance_rank,distinct_customer_cnt)
