...
SELECT DISTINCT LEFT(dm.product_alias, POSITION(' ' IN dm.product_alias) - 1)                AS product_sku
              ...
              , FIRST_VALUE(datetime_start::DATE)
                            OVER (PARTITION BY dm.product_alias ORDER BY datetime_start ASC) AS datetime_start
              , FIRST_VALUE(datetime_end::DATE)
                            OVER (PARTITION BY dm.product_alias ORDER BY datetime_end DESC)  AS datetime_end
...
WHERE featured_product_location_id IN (23921,34821);
...
FROM gfb.gfb_order_line_data_set_ship_date fol
         LEFT JOIN _two_for_one_skus tfo
                   ON tfo.product_sku = fol.product_sku
                       ...
                       AND tfo.datetime_start <= fol.ship_date
                       AND tfo.datetime_end > fol.ship_date
WHERE fol.order_classification = 'product order'
  AND fol.ship_date >= $start_date
...
CREATE OR REPLACE TRANSIENT TABLE gfb.dos_083_01_clearance_reporting_shipped_date AS
SELECT DISTINCT (CASE
              ...
              , prod_info.sub_brand
              , COALESCE(s."Shipped Date", i.inventory_date)                                                    AS "Shipped Date"
              , s."Is_Clearance"
    ...
    END)                                                                                                       AS tier
              ...
              , COUNT(s.product_sku)
                      OVER (PARTITION BY s.business_unit, s.region, s.country, s."Shipped Date", s.product_sku) AS coef
...
FROM _sales s
    ...
    AND i.inventory_date = s."Shipped Date"
         JOIN gfb.merch_dim_product prod_info
