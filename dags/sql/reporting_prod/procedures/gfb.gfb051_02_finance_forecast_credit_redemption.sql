...
where
    ...
    and datediff(month, dv.FIRST_ACTIVATING_COHORT, cap.MONTH_DATE) + 1 > 0
...
select
    ...
    ,mca.REDEEMED_AMOUNT_LOCAL as redeemed_amount
from REPORTING_PROD.GFB.GFB_MEMBERSHIP_CREDIT_ACTIVITY mca
join _month_date md
    on md.MONTH_DATE = date_trunc(month, mca.REDEEMED_ORDER_DATE)
...
where
    ...
    and datediff(month, dv.FIRST_ACTIVATING_COHORT, md.MONTH_DATE) + 1 > 0;


create or replace temporary table _redemption_rate as
select
    bop.BUSINESS_UNIT
    ,bop.REGION
    ,bop.COUNTRY
    ...
    ,red.vip_count * 1.0/bop.bop_vip as vip_redeemed_rate
    ,red.credit_redeemed_count * 1.0/red.vip_count as redeemed_credit_count_per_vip
    ,red.credit_redeemed_amount * 1.0/red.vip_count as redeemed_credit_amount_per_vip
    ,bop.bop_vip
    ,red.vip_count
    ,red.credit_redeemed_count
    ,red.credit_redeemed_amount
from
(
    select
        bop.BUSINESS_UNIT
...
(
    select
        ...
        ,count(cr.CREDIT_ID) as credit_redeemed_count
        ,sum(cr.redeemed_amount) as credit_redeemed_amount
    from _credit_redemption cr
    group by
        cr.BUSINESS_UNIT
        ,cr.REGION
        ,cr.country
...
create or replace temporary table _redemption_rate_coef as
select
    ly.BUSINESS_UNIT
    ,ly.REGION
    ,ly.COUNTRY
    ,ty.redeemed_credit_count_per_vip * 1.0/ly.redeemed_credit_count_per_vip as redeemed_credit_count_per_vip_coef
    ,ty.redeemed_credit_amount_per_vip * 1.0/ly.redeemed_credit_amount_per_vip as redeemed_credit_amount_per_vip_coef
    ,ty.vip_redeemed_rate * 1.0/ly.vip_redeemed_rate as vip_redeemed_rate_coef
from
(
    select
        rr.BUSINESS_UNIT
        ...
        ,sum(rr.credit_redeemed_count) * 1.0/sum(rr.vip_count) as redeemed_credit_count_per_vip
        ,sum(rr.credit_redeemed_amount) * 1.0/sum(rr.vip_count) as redeemed_credit_amount_per_vip
        ,sum(rr.vip_count) * 1.0/sum(rr.bop_vip) as vip_redeemed_rate
    from _redemption_rate rr
    where
        rr.MONTH_DATE >= dateadd(month, -12, dateadd(year, -1, $end_date))
...
(
    ...
    from _redemption_rate rr
    where
        rr.MONTH_DATE >= dateadd(month, -12, $end_date)
...
create or replace temporary table _credit_redemption_forecast as
select
    bvt.BUSINESS_UNIT
    ,bvt.REGION
    ,bvt.COUNTRY
    ...
    ,sum(
            rate.redeemed_credit_amount_per_vip
            * rate.vip_redeemed_rate
            * cbrc.redeemed_credit_amount_per_vip_coef
            * cbrc.vip_redeemed_rate_coef
        ) as forecast_credit_redeemed_amount
...
(
    select
        ...
        ,cbr.COUNTRY
        ,cbr.tenure
        ,avg(cbr.redeemed_credit_count_per_vip) as redeemed_credit_count_per_vip
        ,avg(cbr.redeemed_credit_amount_per_vip) as redeemed_credit_amount_per_vip
        ,avg(cbr.vip_redeemed_rate) as vip_redeemed_rate
    from _redemption_rate cbr
    where
...
left join _redemption_rate_coef cbrc
...
CREATE OR REPLACE TRANSIENT TABLE REPORTING_PROD.GFB.gfb051_02_finance_forecast_credit_redemption as
select
    cbf.BUSINESS_UNIT
    ,cbf.REGION
    ,dateadd(year, 1, r.DATE) as date
    ,sum(cbf.forecast_credit_redeemed_amount * r.CREDIT_REDEEMED_RATE_DAILY) as forecast_credit_redeemed_amount
from _credit_redemption_forecast cbf
