-- set start_date_ly = '2020-11-01';
-- set end_date_ly = '2021-01-01';
SET start_date_ty = '2024-11-01';
...
SELECT olp.business_unit
    ...
    END)                                                                       AS date
     ...
     , IFF(SUM(olp.total_product_revenue) = 0, 0,
           (SUM(olp.cash_collected_amount)) /
           (SUM(olp.total_product_revenue) + SUM(olp.total_shipping_revenue))) AS cash_percent_order
     , SUM(olp.token_product_revenue)                                          AS token_product_revenue
...
WHERE (olp.order_date >= $start_date_ty AND olp.order_date < $end_date_ty)
     ...
     (
         ...
         WHERE a.order_type_id IN (33, 34, 23, 13, 32)
           AND (
                 (
                             ds.store_region = 'NA'
                         AND DATE_TRUNC(DAY, o.datetime_added) = CURRENT_DATE()
                     )
                 OR
                 (
                             ds.store_region = 'EU'
                         AND DATE_TRUNC(DAY, DATEADD(HOUR, 9, o.datetime_added)) = CURRENT_DATE()
                     )
             )
...
WHERE m.datetime_activated >= $start_date_ty
  AND c.email NOT LIKE '%@test%'
  AND c.email NOT LIKE '%@example%'
  AND c.email NOT LIKE '%@fkqa'
GROUP BY UPPER(ds.store_brand)
...
FROM lake_jfb_view.ultra_merchant.membership m
         ...
         JOIN reporting_prod.gfb.vw_store ds
              ON ds.store_id = m.store_id
WHERE m.datetime_added >= $start_date_ty
  AND c.email NOT LIKE '%@test%'
  AND c.email NOT LIKE '%@example%'
  AND c.email NOT LIKE '%@fkqa'
GROUP BY UPPER(ds.store_brand)
...
CREATE OR REPLACE TRANSIENT TABLE reporting_prod.gfb.gfb044_holiday_data_set AS
SELECT o.business_unit
     , o.region
     , o.date
     , o.data_set_filter
...
UNION
SELECT a.*
FROM reporting_prod.gfb.gfb044_holiday_data_set_hist a;
