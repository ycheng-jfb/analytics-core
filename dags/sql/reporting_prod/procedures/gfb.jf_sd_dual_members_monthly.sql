...
CREATE OR REPLACE TEMPORARY TABLE _jf_sd_customers AS
...
WHERE s.store_brand_abbr IN ('JF','SD')
  AND s.store_country = 'US';

CREATE OR REPLACE TEMPORARY TABLE _customer_credits_jf_sd AS
...
FROM edw_prod.data_model_jfb.fact_credit_event fce
     JOIN _jf_sd_customers c
          ON c.customer_id = fce.customer_id
GROUP BY c.customer_id,
         c.activity_month,
         c.store_brand_abbr;

CREATE OR REPLACE TEMPORARY TABLE _jf_sd_data AS
...
FROM _jf_sd_customers c
     ...
     LEFT JOIN _customer_credits_jf_sd cc
...
SELECT COALESCE(jf.activity_month, sd.activity_month) AS activity_month,
       ...
       sd.product_order_count                         AS sd_orders,
       jf.product_order_unit_count                    AS jf_units,
       sd.product_order_unit_count                    AS sd_units,
       jf.product_net_revenue                         AS jf_product_net_revenue,
       sd.product_net_revenue                         AS sd_product_net_revenue,
       jf.billing_cash_net_revenue                    AS jf_billing_net_revenue,
       sd.billing_cash_net_revenue                    AS sd_billing_net_revenue,
       jf.credits_redeemed                            AS jf_credits_redeemed,
       sd.credits_redeemed                            AS sd_credits_redeemed,
       jf.credits_redeemed_till_activity_month        AS jf_credits_redeemed_till_activity_month,
       sd.credits_redeemed_till_activity_month        AS sd_credits_redeemed_till_activity_month,
       jf.outstanding_credits_amount                  AS jf_outstanding_credits,
       sd.outstanding_credits_amount                  AS sd_outstanding_credits,
       CASE
           WHEN jf_orders > 0 AND sd_orders = 0 THEN 'JF order only'
           WHEN sd_orders > 0 AND jf_orders = 0 THEN 'SD order only'
           WHEN sd_orders > 0 AND jf_orders > 0 THEN 'ordered in both'
           WHEN jf_orders = 0 AND sd_orders = 0 THEN 'no orders' END AS jf_sd_order_category,
       CASE
           WHEN jf_billing_net_revenue > 0 AND sd_billing_net_revenue = 0 THEN 'JF billing only'
           WHEN sd_billing_net_revenue > 0 AND jf_billing_net_revenue = 0 THEN 'SD billing only'
           WHEN sd_billing_net_revenue > 0 AND jf_billing_net_revenue > 0 THEN 'billed in both'
           WHEN jf_billing_net_revenue = 0 AND sd_billing_net_revenue = 0
               THEN 'no billings' END                                AS jf_sd_billing_category,
       CASE
           WHEN jf_credits_redeemed > 0 AND sd_credits_redeemed = 0 THEN 'JF redemption only'
           WHEN sd_credits_redeemed > 0 AND jf_credits_redeemed = 0 THEN 'SD redemption only'
           WHEN sd_credits_redeemed > 0 AND jf_credits_redeemed > 0 THEN 'redeemed in both'
           WHEN jf_credits_redeemed = 0 AND sd_credits_redeemed = 0
               THEN 'no redemptions' END                             AS jf_sd_redemption_category
FROM _jf_sd_data jf
     JOIN _jf_sd_data sd
          ON jf.activity_month = sd.activity_month
              AND jf.email = sd.email
              AND jf.store_brand_abbr = 'JF'
              AND sd.store_brand_abbr = 'SD';

CREATE OR REPLACE TRANSIENT TABLE gfb.jf_sd_dual_members_monthly AS
SELECT activity_month,
       jf_sd_order_category,
       jf_sd_billing_category,
       jf_sd_redemption_category,
       COUNT(DISTINCT COALESCE(jf_customer_id, sd_customer_id))                  AS _crossover_count,
---order metrics
       COUNT(DISTINCT CASE
                          WHEN jf_orders > 0 OR sd_orders > 0
                              THEN COALESCE(jf_customer_id, sd_customer_id) END) AS product_order_customer_count,
       COUNT(DISTINCT CASE
                          WHEN jf_sd_order_category = 'no orders'
                              THEN COALESCE(jf_customer_id, sd_customer_id) END) AS no_orders_customer_count,
       SUM(jf_orders + sd_orders)                                                AS orders,
       SUM(jf_units + sd_units)                                                  AS units,
       SUM(jf_product_net_revenue + sd_product_net_revenue)                      AS product_net_rev,
---billing metrics
       COUNT(DISTINCT CASE
                          WHEN jf_billing_net_revenue > 0 OR sd_billing_net_revenue > 0
                              THEN COALESCE(jf_customer_id, sd_customer_id) END) AS billing_customer_count,
       SUM(jf_billing_net_revenue + sd_billing_net_revenue)                      AS billing_net_rev,
--redemption metrics
       COUNT(DISTINCT CASE
                          WHEN jf_credits_redeemed > 0 OR sd_credits_redeemed > 0
                              THEN COALESCE(jf_customer_id, sd_customer_id) END) AS redemption_customer_count,
       SUM(jf_credits_redeemed + sd_credits_redeemed)                            AS redemption_rev
