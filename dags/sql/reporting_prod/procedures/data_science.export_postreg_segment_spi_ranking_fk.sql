...
CREATE OR REPLACE TEMPORARY TABLE _identity as (
    select
        case when try_TO_NUMBER(userid) is not null then userid end as user_id,
...
create or replace temporary table _raw_impressions as (
    ...
    from lake.segment_gfb.javascript_fabkids_PRODUCT_LIST_VIEWED so
...
-- only using grid_all so renaming to _merge and removing union
create or replace temp table _merge as(
