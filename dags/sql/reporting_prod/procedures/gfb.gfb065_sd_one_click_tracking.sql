...
create or replace temporary table _cal_vip as
select
    dv.BUSINESS_UNIT
    ,dv.REGION
    ,dv.COUNTRY
...
create or replace temporary table _membership_cancel_clicked as
select distinct
...
where
    bc.properties_name = 'One Click Cancel - Cancel Membership'
    and bc.originaltimestamp::DATE >= $start_date;


create or replace temporary table _membership_cancel_viewed as
select distinct
...
where
    bv.properties_name = 'One Click Cancel - Cancel Membership'
    and bv.originaltimestamp::DATE >= $start_date;


create or replace temporary table _skip_clicked as
select distinct
    bc.properties_customer_id AS customer_id
    ,date_trunc(month, bc.originaltimestamp::DATE) as month_date
from LAKE.SEGMENT_GFB.JAVASCRIPT_SHOEDAZZLE_BUTTON_CLICKED bc
join _cal_vip cv
    on cv.CUSTOMER_ID = bc.properties_customer_id
where
    bc.properties_name = 'One Click Cancel - Skip The Month Confirmed'
    and bc.originaltimestamp::DATE >= $start_date;


create or replace temporary table _skip_viewd as
select distinct
    bv.properties_customer_id AS customer_id
    ,date_trunc(month, bv.originaltimestamp::DATE) as month_date
from LAKE.SEGMENT_GFB.JAVASCRIPT_SHOEDAZZLE_BUTTON_VIEWED bv
join _cal_vip cv
    on cv.CUSTOMER_ID = bv.properties_customer_id
where
    bv.properties_name = 'One Click Cancel - Skip The Month'
    and bv.originaltimestamp::DATE >= $start_date;


create or replace temporary table _customer_online_cancellation as
SELECT distinct
    cv.CUSTOMER_ID
...
create or replace transient table REPORTING_PROD.GFB.gfb065_sd_one_click_tracking as
select
    bv.BUSINESS_UNIT
    ,bv.REGION
    ,bv.COUNTRY
