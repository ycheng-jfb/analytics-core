create or replace temporary table _customer_monthly_value as
select
    dv.BUSINESS_UNIT
    ,dv.REGION
    ,clvm.MONTH_DATE
...
create or replace temporary table _customer_tenure_segment as
select distinct
    cmv.CUSTOMER_ID
    ,cmv.tenure as segment_name
    ,cmv.tenure_num
    ,cmv.VIP_COHORT_MONTH_DATE
from _customer_monthly_value cmv
where
...
create or replace transient table REPORTING_PROD.GFB.gfb001_03_tenure_block_lifetime_value as
select
    a.BUSINESS_UNIT
    ,a.REGION
    ,a.tenure
...
from _customer_monthly_value a
join _customer_tenure_segment cts
