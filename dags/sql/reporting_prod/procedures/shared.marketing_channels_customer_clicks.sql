SET email_clicks_watermark = (
    SELECT IFNULL(DATEADD(hour, -48, (
        SELECT MAX(event_time)
        FROM reporting_prod.shared.marketing_channels_customer_clicks
        WHERE source_table = 'lake.iterable.email_clicks'
    )), '1990-01-01')
);
SET push_opens_watermark = (
    SELECT IFNULL(DATEADD(hour, -48, (
        SELECT MAX(event_time)
        FROM reporting_prod.shared.marketing_channels_customer_clicks
        WHERE source_table = 'lake.iterable.push_opens'
    )), '1990-01-01')
);
SET sms_clicks_watermark = (
    SELECT IFNULL(DATEADD(hour, -48, (
        SELECT MAX(event_time)
        FROM reporting_prod.shared.marketing_channels_customer_clicks
        WHERE source_table = 'lake.iterable.sms_clicks'
    )), '1990-01-01')
);
...
SELECT
    ...
    es.project_id                AS source_store_id,
    NULL                         AS contact_id,
   'email'                       AS type,
    ...
    LOWER(REGEXP_SUBSTR(REGEXP_REPLACE(es.template_name, '[^\\x20-\\x7E]', '', 1), '[^_]*$')),NULL) AS version_name
...
WHERE
    es.created_at > $email_clicks_watermark;
...
SELECT
    TRY_TO_NUMBER(u.user_id)   AS customer_id,
    s.store_group_id           AS store_group_id,
    po.project_id              AS source_store_id,
    NULL                       AS contact_id,
    'push'                     AS type,
    ...
    LOWER(REGEXP_SUBSTR(REGEXP_REPLACE(po.template_name, '[^\\x20-\\x7E]', '', 1), '[^_]*$')),NULL) AS version_name
...
LEFT JOIN LAKE_CONSOLIDATED_VIEW.ULTRA_MERCHANT.STORE s
    ON cust.store_id = s.store_id
WHERE
    po.created_at > $push_opens_watermark;
...
LEFT JOIN LAKE_CONSOLIDATED_VIEW.ULTRA_MERCHANT.STORE s
    ON cust.store_id = s.store_id
WHERE
    sms.created_at > $sms_clicks_watermark;


MERGE INTO reporting_prod.shared.marketing_channels_customer_clicks AS target
USING (
/*
 !!! UNCOMMENT IF YOU NEED TO REBUILD THE TABLE !!!
    ...
    b AS (SELECT id, sailthru_account_id, name
                FROM (SELECT id,
                             sailthru_account_id,
                             name,
...
*/
    SELECT
        ...
        A.version_name
    FROM (
        SELECT * FROM _email_clicks_delta
        UNION
        SELECT * FROM _push_opens_delta
        UNION
        SELECT * FROM _sms_clicks_delta
/*
        SELECT
            ...
            'email'                     AS type,
            es.event_time,
            'lake.emarsys.email_clicks' AS source_table,
        ...
        FROM lake_view.emarsys.email_clicks AS es
        ...
        WHERE
            es.event_time > (
                SELECT IFNULL((
                    SELECT MAX(event_time)
                    FROM reporting_prod.shared.marketing_channels_customer_clicks
                    WHERE source_table = 'lake.emarsys.email_clicks'
                ), '1990-01-01')
            )
        ...
        SELECT
            ...
            'push'                    AS type,
            ps.event_time,
            'lake.emarsys.push_opens' AS source_table,
            ...
            NULL                      AS message_name
        FROM lake_view.emarsys.push_opens AS ps
        LEFT JOIN lake.emarsys.customer_store_mapping AS sm
        ...
        WHERE
            ps.event_time > (
                SELECT IFNULL((
                    SELECT MAX(event_time)
                    FROM reporting_prod.shared.marketing_channels_customer_clicks
                    WHERE source_table = 'lake.emarsys.push_opens'
                ), '1990-01-01')
            )
        ...
        SELECT
            ...
            'email'                                     AS type,
            c.value:ts::TIMESTAMP                       AS event_time,
            ...
            NULL                                        AS version_name,
            NULL                                        AS message_name
        FROM lake.sailthru.data_exporter_message_blast AS mb
        LEFT JOIN reporting.crm.sailthru_data_exporter_profile_summary AS ps
            ON mb.profile_id = ps.id
        LEFT JOIN lake.sailthru.account_mapping AS am
            ON mb.sailthru_account_id = am.sailthru_account_id
        LEFT JOIN b
            ON mb.blast_id = b.id
            AND mb.sailthru_account_id = b.sailthru_account_id,
        LATERAL FLATTEN (input => clicks) AS c
        WHERE mb.send_time > $event_time_after
            AND mb.send_time <= $event_time_equal_or_before
        ...
        WHERE
            ...
            AND UPPER(sms.type) = 'MESSAGE_LINK_CLICK'
 */
        ) AS A
/*
