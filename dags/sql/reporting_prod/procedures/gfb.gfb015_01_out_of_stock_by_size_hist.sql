...
select distinct
    ...
    ,(case
        ...
        then id.QTY_ONHAND else 0 end) as total_inventory
...
left join LAKE_VIEW.sharepoint.GFB_MERCH_CLEARANCE_SKU_LIST mcsl
    ...
    and (cast(id.INVENTORY_DATE as date) between mcsl.START_DATE and coalesce(mcsl.END_DATE, current_date()))
left join (
    SELECT DISTINCT
        MASTER_PRODUCT_ID,
    ...
    and (id.INVENTORY_DATE between pph.EFFECTIVE_START_DATETIME and pph.EFFECTIVE_END_DATETIME)
where
    (mdp.SHARED = mdp.BUSINESS_UNIT or mdp.SHARED is null)
    and id.COUNTRY != 'CA' /* since we do not have Canada warehouse any more. */
    and id.INVENTORY_DATE >= $start_date
...
create or replace transient table reporting_prod.gfb.gfb015_01_out_of_stock_by_size_hist as
select
    oss.*
...
left join _core_size_broken csb
    ...
    and csb.INVENTORY_DATE = oss.INVENTORY_DATE
