...
CREATE OR REPLACE TEMPORARY TABLE _fk_members_pre_scaffold AS
SELECT DISTINCT dc.email,
                dc.customer_id,
                MAX(IFF(fa.activation_local_datetime IS NULL, '9999-12-31',
                        fa.activation_local_datetime))   AS activation_date,
...
FROM edw_prod.data_model_jfb.fact_activation fa
     ...
     JOIN edw_prod.data_model_jfb.dim_store ds
          ON fa.store_id = ds.store_id
              AND store_brand_abbr = 'FK'
GROUP BY dc.email,
         dc.customer_id;

CREATE OR REPLACE TEMPORARY TABLE _fk_members AS
SELECT DISTINCT m.activity_month,
                ps.email,
                ps.customer_id,
                ps.activation_date,
                ps.cancellation_date,
                DATEDIFF('month', ps.cancellation_date, m.activity_month) AS cancel_tenure
FROM _fk_members_pre_scaffold ps
     JOIN _month_scaffold m
          ON m.activity_month >= DATE_TRUNC('month', activation_date)
              AND m.activity_month < DATE_TRUNC('month', cancellation_date);
...
CREATE OR REPLACE TEMPORARY TABLE _credits_redeemed_fk AS
SELECT fk.customer_id,
       fk.activity_month,
       COUNT(IFF(credit_activity_type = 'Redeemed' AND
                 activity_month = DATE_TRUNC('month', credit_activity_local_datetime::DATE), credit_id,
                 0)) AS credits_redeemed
FROM edw_prod.data_model_jfb.fact_credit_event fce
     JOIN _fk_members fk
          ON fk.customer_id = fce.customer_id
              AND fk.activity_month = DATE_TRUNC('month', fce.credit_activity_local_datetime::DATE)
GROUP BY fk.customer_id,
         fk.activity_month;
...
CREATE OR REPLACE TEMPORARY TABLE _fk_fso AS
SELECT fk.activity_month,
       fk.email,
       fk.customer_id,
       fk.activation_date,
       fk.cancellation_date,
       fk.cancel_tenure,
       SUM(fso.product_order_count)                                                 AS product_orders,
       SUM(fso.product_order_unit_count)                                            AS product_order_units,
       SUM(fso.product_gross_revenue)                                               AS product_gross_revenue,
...
FROM _fk_members fk
     LEFT JOIN edw_prod.analytics_base.finance_sales_ops fso
               ON fk.customer_id = edw_prod.stg.udf_unconcat_brand(fso.customer_id)
                   AND DATE_TRUNC('month', fso.date) = fk.activity_month
                   AND fso.date_object = 'placed'
                   AND fso.currency_object = 'usd'
     LEFT JOIN _credits_redeemed_fk cr
               ON fk.customer_id = cr.customer_id
                   AND cr.activity_month = fk.activity_month
GROUP BY fk.activity_month,
         fk.email,
         fk.customer_id,
         fk.activation_date,
         fk.cancellation_date,
...
FROM _jf_members jf
     JOIN _fk_members fk
          ON TRIM(LOWER(jf.email)) = TRIM(LOWER(fk.email));

CREATE OR REPLACE TEMPORARY TABLE _dual_members AS
SELECT DISTINCT COALESCE(jf.activity_month, fk.activity_month) AS activity_month,
                COALESCE(jf.email, fk.email)                   AS email
FROM _crossover_customer_ids cc
     JOIN _jf_members jf
          ON cc.jf_customer_id = jf.customer_id
     JOIN _fk_members fk
          ON cc.fk_customer_id = fk.customer_id
              AND jf.activity_month = fk.activity_month;
...
SELECT DISTINCT CASE
                ...
                jf.cancel_tenure,
                jf.product_orders,
                jf.product_order_units,
                jf.product_gross_revenue,
                jf.product_net_revenue,
                jf.billing_revenue,
                jf.billings_count,
                jf.credits_redeemed,
                jf.credit_redeemed_amount,
                jf.membership_cancels
FROM _jf_fso jf
...
SELECT DISTINCT CASE
                    WHEN dm.email IS NOT NULL THEN 'Dual Membership'
                    WHEN fk.activity_month > COALESCE(ldm.max_month, '9999-12-31') AND
                         COALESCE(ldm.max_month, '9999-12-31') != '2023-10-01' THEN 'Down from 2 to 1'
                    END                                                      AS activity_flag,
                COALESCE(ldm.max_month, DATE_TRUNC('month', CURRENT_DATE())) AS max_month,
                fk.activity_month,
                ...
                fk.cancel_tenure,
                fk.product_orders,
                fk.product_order_units,
                fk.product_gross_revenue,
                fk.product_net_revenue,
                fk.billing_revenue,
                fk.billings_count,
                fk.credits_redeemed,
                fk.credit_redeemed_amount,
                fk.membership_cancels
FROM _fk_fso fk
     JOIN _crossover_customer_ids cc
          ON fk.email = cc.email
     LEFT JOIN _dual_members dm
               ON fk.activity_month = dm.activity_month
                   AND fk.email = dm.email
     LEFT JOIN _last_dual_month ldm
               ON cc.email = ldm.email;

CREATE OR REPLACE TEMPORARY TABLE _customer_activity_final AS
SELECT activity_flag,
       max_month,
       activity_month,
       email,
       customer_id,
...
CREATE OR REPLACE TRANSIENT TABLE gfb.jf_fk_downgraded_members_monthly AS
SELECT activity_month,
       max_month,
       cancel_tenure,
