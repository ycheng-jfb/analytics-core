CREATE TABLE IF NOT EXISTS reporting_prod.membership.monthly_ecom_passive_cancels
(
...
SET first_of_the_month = DATE_TRUNC('MONTH',$LowWatermarkDate); --first day of the month
SET upper_date = DATEADD(YEAR,-1,$first_of_the_month); --look back 365 days in time from the first of the month
...
WHERE dosc.order_classification_l2 NOT IN ('Credit Billing', 'Exchange', 'Reship','Token Billing')
    AND fo.order_local_datetime >= $upper_date
    AND fo.order_local_datetime < $first_of_the_month
...
WHERE s.datetime_added >= $upper_date
    AND s.datetime_added < $first_of_the_month
...
WHERE dcat.customer_action_type = 'Skipped Month'
    ...
    AND fca.customer_action_local_datetime < $first_of_the_month;
...
WHERE $first_of_the_month >= fme.event_start_local_datetime
    AND $first_of_the_month < fme.event_end_local_datetime
    AND fme.membership_event_type = 'Activation';

DELETE FROM reporting_prod.membership.monthly_ecom_passive_cancels
WHERE passive_month_date = $first_of_the_month;

INSERT INTO reporting_prod.membership.monthly_ecom_passive_cancels
(
    membership_id,
    customer_id,
    passive_month_date,
...
SELECT
    m.membership_id,
    cs.customer_id,
    $first_of_the_month AS passive_month_date,
...
WHERE is_current_month = 1
	AND passive_month_date < $first_of_the_month; -- Set last month records to is_current_month flag to 0
