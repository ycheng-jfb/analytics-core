CREATE OR REPLACE VIEW reporting_prod.jira.jira_capex AS
SELECT DISTINCT i.id                               AS issue_id,
                ...
                CASE
                    WHEN i.component LIKE '%Data Analytics - Data Engineering%' THEN 'Data Engineering'
                ...
                CASE
                    WHEN i.capex = '14800' THEN 'Capital'
                ...
                e.release_notes                    AS epic_release_notes,
                p.meta_create_datetime             AS project_meta_create_datetime
FROM reporting_prod.jira.jira_issues i
    LEFT JOIN reporting_prod.jira.jira_epics e ON i.epic_link = e.key
    LEFT JOIN lake_view.jira.project p ON i.project_id = p.id
    LEFT OUTER JOIN reporting_prod.jira.jira_first_close_date_mv2 ad ON i.id = ad.issue_id
WHERE i.project_id IN
      (10003, 10006, 10008, 10014, 10018, 10019, 10021, 10023, 10025, 10027, 10029, 10031, 10032, 10033, 10035, 10036,
  ...
  AND TO_DATE(ad.first_closed_date) >= DATEADD('MONTH', -1, DATE_TRUNC('MONTH', CURRENT_DATE()))
  AND TO_DATE(ad.first_closed_date) <= TO_DATE(DATEADD(D, -1, TO_VARCHAR(CURRENT_DATE(), 'yyyy-mm-01')))
GROUP BY ALL;
