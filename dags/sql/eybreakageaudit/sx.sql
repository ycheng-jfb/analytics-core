...
CREATE OR REPLACE TRANSIENT TABLE audit.order_2023_sx AS
...
CREATE OR REPLACE TRANSIENT TABLE audit.membership_store_credit_2023_sx AS
SELECT msc.*, CURRENT_TIMESTAMP as table_loaded_datetime
FROM lake_sxf.ultra_merchant.membership_store_credit msc
JOIN lake_sxf.ultra_merchant.store_credit sc ON sc.store_credit_id = msc.store_credit_id
JOIN lake_sxf.ultra_merchant.customer c ON sc.customer_id = c.customer_id
WHERE store_id IN (
    SELECT store_id
    FROM edw_prod.data_model.dim_store
    WHERE store_brand_abbr = 'SX'
...
CREATE OR REPLACE TRANSIENT TABLE audit.membership_credit_2023_sx AS
SELECT sc.*, CURRENT_TIMESTAMP AS table_loaded_datetime
FROM lake_sxf.ultra_merchant.store_credit sc
JOIN lake_sxf.ultra_merchant.store_credit_reason sr ON sc.STORE_CREDIT_REASON_ID = sr.STORE_CREDIT_REASON_ID
JOIN lake_sxf.ultra_merchant.customer cust ON sc.customer_id = cust.customer_id
...
CREATE OR REPLACE TRANSIENT TABLE audit.issued_orders_2023_sx AS
