CREATE OR REPLACE VIEW LAKE_VIEW.PINTEREST.SPEND_AD30 AS
...
from LAKE_FIVETRAN.MED_PINTEREST_AD30_V1.PIN_PROMOTION_REPORT P
LEFT JOIN (select  id, name, country from LAKE_FIVETRAN.MED_PINTEREST_AD30_V1.ADVERTISER_HISTORY
...
select ADVERTISER_ID,
    ...
    convert_timezone('America/Los_Angeles',P._FIVETRAN_SYNCED) AS META_CREATE_DATETIME,
    convert_timezone('America/Los_Angeles',P._FIVETRAN_SYNCED) as META_UPDATE_DATETIME
from LAKE_FIVETRAN.MED_PINTEREST_AD30_V1.PRODUCT_GROUP_REPORT P
LEFT JOIN (select  id, name, country from LAKE_FIVETRAN.MED_PINTEREST_AD30_V1.ADVERTISER_HISTORY
qualify row_number() over (partition by id order by updated_time desc ) = 1) AH on P.ADVERTISER_ID=AH.id;
