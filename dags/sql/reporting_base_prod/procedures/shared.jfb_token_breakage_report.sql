...
CREATE OR REPLACE TEMPORARY TABLE _jfb_token_base AS
SELECT brand,
       region,
       country,
       original_credit_tender,
...
CREATE OR REPLACE TEMPORARY TABLE _append_jfb_token AS
WITH _activity_amount AS (SELECT *
                          FROM _jfb_token_base
     ...
     _expected_redemption_rate AS (SELECT brand,
                                          ...
                                          SUM(redeemed_percent)
                                              OVER (PARTITION BY brand, country ORDER BY credit_tenure ASC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) cumulative_redeemed_percent
                                   FROM (SELECT brand,
                                                country,
                                                credit_tenure,
                                                NVL(SUM(redeemed)
     ...
     _expected_redemption_rate_13 AS (SELECT brand,
                                             country,
                                             credit_tenure,
                                             SUM(redeemed_percent)
                                                 OVER (PARTITION BY brand, country ORDER BY credit_tenure ASC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
                                                 +
                                             IFF(DECODE(brand || ' ' || country, 'JustFab US',
                                                        $month_thru BETWEEN '2023-08-01' AND '2024-07-01',
                                                        $month_thru BETWEEN '2023-10-01' AND '2024-09-01'), 0.01, 0) cumulative_redeemed_percent
                                      FROM (SELECT brand,
                                                   country,
                                                   credit_tenure,
     ...
     _expected_unredeem_rate AS (SELECT brand,
                                        ...
                                        IFF(DECODE(brand || ' ' || country, 'JustFab US',
                                                   $month_thru BETWEEN '2023-08-01' AND '2024-07-01',
                                                   $month_thru BETWEEN '2023-10-01' AND '2024-09-01'), 0.01, 0) cumulative_unredeem_percent
                                 FROM (SELECT brand,
                                              country,
                                              credit_tenure,
...
SELECT a.brand || ' ' || a.country                                                 business_unit,
       ...
       IFF(DATEDIFF(MONTH, original_issued_month, $month_thru) + 1 <= 13,
...
FROM _activity_amount a
         JOIN _expected_redemption_rate r12 ON r12.credit_tenure = 12 and a.brand = r12.brand AND a.country = r12.country
         JOIN _expected_redemption_rate_13 r13 ON r13.credit_tenure = 13  and a.brand = r13.brand AND a.country = r13.country
         JOIN _expected_unredeem_rate ur ON ur.credit_tenure = 13  and a.brand = ur.brand AND a.country = ur.country
GROUP BY a.brand,
         a.region,
         a.country,
         a.original_credit_tender,
...
DELETE FROM shared.jfb_token_breakage_report
where recognition_booking_month = $month_thru;

INSERT INTO shared.jfb_token_breakage_report
