...
select
    ...
    case when account_name = 'Fabletics_CA' then dateadd(hour, -3, cast(concat(date_start,' ', left(hourly_stats_aggregated_by_advertiser_time_zone,8)) as datetime)) --adjust to pst time
       else cast(concat(date_start,' ', left(hourly_stats_aggregated_by_advertiser_time_zone,8)) as datetime)
    end as date_hour,
...
from lake.facebook.ad_insights_by_hour fb
...
left join edw_prod.reference.currency_exchange_rate_by_date lkpusd on am.currency = lkpusd.src_currency
    and fb.date_start = lkpusd.rate_date_pst
...
left join edw_prod.reference.currency_exchange_rate_by_date lkplocal on am.currency = lkplocal.src_currency
    and fb.date_start = lkplocal.rate_date_pst
...
create or replace transient table reporting_base_prod.shared.med61_realtime_acquisition_secondary as
select
    edw.brand,
    edw.region,
    edw.country,
...
alter table reporting_base_prod.shared.med61_realtime_acquisition_secondary set data_retention_time_in_days = 0;
...
create or replace transient table reporting_base_prod.shared.med61_realtime_orders_secondary as
select
    ds.store_brand as brand,
    ds.store_region as region,
    ds.store_country as country,
...
alter table reporting_base_prod.shared.med61_realtime_orders_secondary set data_retention_time_in_days = 0;
