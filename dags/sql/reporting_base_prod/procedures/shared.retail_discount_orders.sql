CREATE OR REPLACE TRANSIENT TABLE reporting_base_prod.shared.retail_discount_order AS
SELECT DATE_TRUNC(MONTH, order_completion_local_datetime::DATE) AS month_date,
       ...
       fo.order_id,
       dp.promo_code,
       IFF((dp.promo_code ILIKE '%team40%' OR dp.promo_code ILIKE '%team50%' OR dp.promo_code ILIKE '%teamtfg%'),
           'Employee Discount',
...
ALTER TABLE reporting_base_prod.shared.retail_discount_order
    SET DATA_RETENTION_TIME_IN_DAYS = 0;

INSERT INTO reporting_base_prod.shared.retail_discount_order_snapshot
SELECT month_date
     , store_brand
     , store_region
     , store_full_name
     ...
     , CURRENT_TIMESTAMP snapshot_datetime
FROM reporting_base_prod.shared.retail_discount_order;

DELETE
FROM reporting_base_prod.shared.retail_discount_order_snapshot
