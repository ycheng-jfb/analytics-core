...
set last_load_datetime = (
    select max(a.session_local_datetime)::date-$delta_window as last_load_datetime
    from reporting_base_prod.shared.session_order_ab_test_detail_optimized a
    join reporting_base_prod.shared.session b
        on a.session_id = b.session_id
    WHERE a.session_local_datetime >= DATEADD(DAY, -$delta_window, CURRENT_DATE())
    );
...
-- CREATE OR REPLACE TRANSIENT TABLE reporting_base_prod.shared.session_order_ab_test_detail_optimized as --comment
SELECT
    m.SESSION_ID
     ,'ALL' AS version
     ,m.TEST_KEY
...
delete from reporting_base_prod.shared.session_order_ab_test_detail_optimized --comment
where session_id in (select session_id from _sessions_last7days);

DELETE FROM reporting_base_prod.shared.session_order_ab_test_detail_optimized
where
    TEST_KEY not in (select test_key from _last_3_months)
        and test_label not in (select test_label from _last_3_months);

insert into reporting_base_prod.shared.session_order_ab_test_detail_optimized --comment
(
SESSION_ID,
TEST_KEY,
AB_TEST_SEGMENT,
...
CREATE OR REPLACE TEMPORARY TABLE _ca_customers as
    (select distinct c.CUSTOMER_ID
     from (select distinct customer_id from reporting_base_prod.shared.session_order_ab_test_detail_optimized where campaign_code in ('TariffFKKi2866','MYVIPSNOOZEOPTION','ONLINECANCELREDESIGN','CANCELFIRSTORDER')) as c
...
--             from reporting_base_prod.shared.SESSION_ORDER_AB_TEST_DETAIL_optimized as d
...
-- from reporting_base_prod.SHARED.SESSION_ORDER_AB_TEST_DETAIL_optimized as s
...
-- from reporting_base_prod.SHARED.SESSION_ORDER_AB_TEST_DETAIL_optimized as a
...
-- join reporting_base_prod.SHARED.SESSION_ORDER_AB_TEST_DETAIL_optimized as b on a.CUSTOMER_ID = b.customer_id
...
--
-- delete from reporting_base_prod.SHARED.SESSION_ORDER_AB_TEST_DETAIL_optimized --deleting ineligible traffic now
--     where
--         edw_prod.stg.udf_unconcat_brand(TEST_FRAMEWORK_ID) = 1245
--         and SESSION_ID not in (select session_id from _eligible_sessions_from_customers_1245);

update reporting_base_prod.SHARED.SESSION_ORDER_AB_TEST_DETAIL_optimized
...
where (
    ...
    AND (
    COALESCE(s.IS_SHOPPING_GRID_VISIT,'') NOT ILIKE 'true'
    OR COALESCE(s.IS_PDP_VISIT,'') NOT ILIKE 'true'
    OR COALESCE(s.IS_ATB_ACTION,'') NOT ILIKE 'true'
    );
/*
--4,482,873 rows, 2024-09-10 22:53:59.603
--3,635,347 rows, 2024-09-10 22:53:59.603
SELECT COUNT(*), MIN(AB_TEST_START_UTC_DATETIME)
...
SET min_watermark = (
    SELECT DATEADD(DAY, -$delta_window, $last_load_datetime)::datetime AS min_watermark
    );
