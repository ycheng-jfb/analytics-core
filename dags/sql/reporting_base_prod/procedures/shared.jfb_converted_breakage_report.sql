...
WITH _activity_amount AS (SELECT *
     ...
     _expected_redemption_rate_max AS (SELECT brand,
                                              ...
                                              credit_tenure,
                                              SUM(redeemed_percent)
                                                  OVER (PARTITION BY brand, country ORDER BY credit_tenure ASC
                                                      ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
                                                  +
                                              IFF(DECODE(brand || ' ' || country, 'JustFab US',
                                                         $month_thru BETWEEN '2023-08-01' AND '2024-07-01',
                                                         $month_thru BETWEEN '2023-10-01' AND '2024-09-01'), 0.01,
                                                  0) cumulative_redeemed_percent
                                       FROM (SELECT brand,
                                                    country,
                                                    credit_tenure,
...
DELETE FROM shared.jfb_converted_breakage_report
where recognition_booking_month = $month_thru;

INSERT INTO shared.jfb_converted_breakage_report
