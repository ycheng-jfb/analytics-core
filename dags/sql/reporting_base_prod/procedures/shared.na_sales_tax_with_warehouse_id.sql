...
CREATE OR REPLACE TRANSIENT TABLE reporting_base_prod.shared.na_sales_tax_with_warehouse_detail AS
SELECT segment,
       order_id,
       refund_id,
       business_unit,
       reporting_base_prod.shared.udf_correct_state_country(state, country, 'country') AS country,
       retail_location,
       month,
       store_id,
       store_type,
...
DELETE
FROM reporting_base_prod.shared.na_sales_tax_with_warehouse_detail
WHERE product_dollars = 0
  AND shipping = 0
  AND tax = 0
  AND total = 0;

INSERT INTO reporting_base_prod.shared.na_sales_tax_with_warehouse_detail_snapshot
SELECT segment,
       order_id,
       refund_id,
       business_unit,
