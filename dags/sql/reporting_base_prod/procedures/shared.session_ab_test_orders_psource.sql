...
CREATE OR REPLACE TABLE _eligible_psource_orders as
-- select
--     s.test_key
--     ,s.test_label
--     ,s.test_group
...
-- join REPORTING_BASE_PROD.SHARED.ORDER_LINE_PSOURCE_PAYMENT as o on o.SESSION_ID = s.SESSION_ID
-- join REPORTING_PROD.SHARED.AB_TEST_BUILDER_GRID_PSOURCES as p on p.test_start_psource = o.PSOURCE
--     and s.test_label = p.test_label
-- where
--     o.ORDER_LOCAL_DATETIME >= s.AB_TEST_START_LOCAL_DATETIME
-- group by 1,2,3,4,5
--
-- union all
...
select
    ...
    ,o.ORDER_ID
    ,count(*) as count
from reporting_base_prod.shared.SESSION_AB_TEST_CMS_FRAMEWORK as s
join REPORTING_BASE_PROD.SHARED.ORDER_LINE_PSOURCE_PAYMENT as o on o.SESSION_ID = s.SESSION_ID
where
    o.ORDER_LOCAL_DATETIME >= s.AB_TEST_START_LOCAL_DATETIME
    and
        (
            (lower(CAMPAIGN_CODE) in ('remix3284', 'remix3284sd') and contains('skip_request', PSOURCE))
                or (CAMPAIGN_CODE = 'FLUSSSVCSTR' and PSOURCE ilike '%search%')
        )
group by 1,2,3,4,5;

CREATE OR REPLACE TEMPORARY TABLE _orders_by_session_psource as
select
...
join _eligible_psource_orders as e on e.ORDER_ID = pp.ORDER_ID
    AND e.SESSION_ID = pp.SESSION_ID
    and e.test_label = s.test_label
    and e.AB_TEST_SEGMENT = s.AB_TEST_SEGMENT
where
    TTL_ORDER_RNK = 1
...
CREATE OR REPLACE TEMPORARY TABLE _vips_from_lead_registrations_psource as
select
...
join _eligible_psource_orders as e on e.ORDER_ID = pp.ORDER_ID
    ...
    and e.AB_TEST_SEGMENT = s.AB_TEST_SEGMENT
where
    s.SESSION_ID not in (select distinct SESSION_ID from _orders_by_session_psource)
...
CREATE OR REPLACE TRANSIENT TABLE reporting_base_prod.shared.session_ab_test_orders_psource as

select * from _orders_by_session_psource

union

select * from _vips_from_lead_registrations_psource;

ALTER TABLE reporting_base_prod.shared.session_ab_test_orders_psource SET DATA_RETENTION_TIME_IN_DAYS = 0;
...
--     ,sessions - sessions_distinct as diff
--     ,sum(orders_total) as orders
-- from reporting_base_prod.shared.session_ab_test_orders_psource
-- group by 1,2,3
-- order by 5 desc;

-- delete from reporting_base_prod.shared.session_ab_test_builder_dev
-- where test_label not in
-- ('FLW Sale | Womens - Sale All - AB Test (Nov 2023) - Control Hero | Womens - Sale All - AB Test (Nov 2023) - Toggle Test'
-- ,'FLM Sale | Mens - Sale All - AB Test (Nov 2023) - Control | Mens - Sale All - AB Test (Nov 2023) - Toggle Test'
-- ,'FLS Womens Sets | Scrubs - Womens Sets AB Test (June 2023) - Marketing Assets - Control | Scrubs - Womens Sets AB Test (June 2023) - Grid View'
-- ,'FLS All Womens | Scrubs - Shop All Women''s - Sort AB Test - Control | Scrubs - Shop All Women''s - Sort AB Test - Best Sellers Test'
-- ,'FLS All Mens | Scrubs - Shop All Mens -  Sort AB Test - Control | Scrubs - Shop All Mens - Sort AB Test - Best Sellers Test')
