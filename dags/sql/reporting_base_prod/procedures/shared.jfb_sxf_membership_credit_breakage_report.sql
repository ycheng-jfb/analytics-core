...
WHERE original_credit_tender = 'Cash'
  AND iff(brand LIKE 'Fabletics%',
          ((original_credit_type = 'Fixed Credit' AND original_credit_reason = 'Membership Credit') OR
           (original_credit_type = 'Variable Credit' AND original_credit_reason = 'Refund')),
...
WITH _activity_amount AS (SELECT *
                          FROM _jfb_sxf_base
                          WHERE original_credit_reason IN ('Membership Credit','Refund')
...
DELETE FROM shared.jfb_sxf_membership_credit_breakage_report
where recognition_booking_month = $month_thru;

INSERT INTO shared.jfb_sxf_membership_credit_breakage_report
