...
FROM reporting_base_prod.shared.fabletics_breakage_report_ssrs_base
...
SELECT currency,
       ...
       blended_credit_cohorts,
       blended_credit_cohorts_type
FROM reporting_base_prod.shared.sxf_breakage_report_ssrs_base
WHERE currency IN ('Local Gross VAT', 'Local Net VAT')
  AND recognition_booking_month = $target_date;
...
SELECT currency,
       ...
       blended_credit_cohorts,
       blended_credit_cohorts_type
FROM reporting_base_prod.shared.jfb_breakage_report_ssrs_base
WHERE currency IN ('Local Gross VAT', 'Local Net VAT')
  AND recognition_booking_month = $target_date;
...
CREATE OR REPLACE TRANSIENT TABLE shared.tax_breakage AS
SELECT currency,
       region,
       display_store,
       issued_year,
...
INSERT INTO shared.tax_breakage_snapshot
SELECT currency,
       region,
       display_store,
       issued_year,
