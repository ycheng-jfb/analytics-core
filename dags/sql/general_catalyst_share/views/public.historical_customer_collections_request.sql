CREATE OR REPLACE SECURE VIEW public.historical_customer_collections_request (
    customer_id,
    customer_segment,
    brand,
    vip_cohort,
...
SELECT DISTINCT fso.customer_id,
    ...
    SUM(product_order_cash_refund_amount) + SUM(product_order_cash_chargeback_amount) +
    ...
    SUM(product_order_variable_gms_cost_amount)
        ...
        + (gc.gfc_back_office_pct * (cash_net_rev))
        + (gc.gms_fixed_pct * (cash_net_rev))
        + (gc.gms_back_office_pct * (cash_net_rev))                                                         AS total_expenses
...
JOIN (
    SELECT *
    FROM reporting_prod.fabletics.gc_fixed_variable_expenses_nov24
    WHERE segment = 'FLNA'
...
GROUP BY fso.customer_id,
    ...
    gc.gfc_fixed_pct,
    gc.gms_fixed_pct,
    gc.gfc_back_office_pct,
    gc.gms_back_office_pct
UNION
SELECT DISTINCT fso.customer_id,
    CASE
    WHEN st.store_brand = 'Fabletics' AND c.is_scrubs_customer = TRUE AND UPPER(c.gender) != 'M'
