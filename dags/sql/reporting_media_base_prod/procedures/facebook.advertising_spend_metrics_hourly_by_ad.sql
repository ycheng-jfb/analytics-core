...
SET meta_update_datetime = (SELECT MAX(META_UPDATE_DATETIME) FROM LAKE_VIEW.FACEBOOK.AD_INSIGHTS_BY_HOUR);

CREATE OR REPLACE TRANSIENT TABLE reporting_media_base_prod.FACEBOOK.ADVERTISING_SPEND_METRICS_HOURLY_BY_AD AS
SELECT
    STORE_ID,
    ACCOUNT_ID,
    CAMPAIGN_ID,
...
FROM
    ...
    SELECT
        ...
        AD_ID,
        date AS DATE,
        LEFT(HOURLY_STATS_AGGREGATED_BY_ADVERTISER_TIME_ZONE,2) AS HOUR,
