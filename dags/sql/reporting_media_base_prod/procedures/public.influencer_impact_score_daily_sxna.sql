...
set daily_data_start_date ='2021-02-06';


-- daily engagements data
create or replace temporary table _ciq_impact_score as
select
    campaignname,
    publisherid,
    PUBLISHERNAME,
...
from reporting_media_base_prod.creatoriq.campaign_activity_cleansed
    where DATESUBMITTED >= '2020-11-05'
    and lower(CAMPAIGNNAME) like any ('%savage%', '%sx%')
    and not lower(CAMPAIGNNAME)  like any ('%uk%', '%fr%', '%france%', '%germany%', '%eu%')
order by publishername asc, POSTID asc, META_TO_DATETIME asc, META_FROM_DATETIME asc;

create or replace temporary table _impact_score_per_influencer_per_day as
select campaignname,
       publisherid,
       PUBLISHERNAME,
       postid,
...
create or replace temporary table _impact_score_per_influencer_by_post as
select
...
--             LINK_TO_POST,
    ...
    where lower(CAMPAIGN_NAME) like any ('%savage%', '%sx%')
    and not lower(CAMPAIGN_NAME)  like any ('%uk%', '%fr%', '%france%', '%germany%', '%eu%')
    and POST_DATETIME >= '2019-01-01'
    order by PUBLISHER_NAME, hours_since_post;


-- get general decay curve
create or replace temporary table _general_decay_curve as
    WITH t1 as
        ...
        from _impact_score_per_influencer_by_post post
        JOIN _impact_score_per_influencer_per_day d on d.publisherid = post.publisher_id and d.postid = post.post_id
        where d.days_after_post <= 7
        and impact_score_by_post > 0
        and impact_score_daily > 0
        and d.days_after_post >= 0
        and d.metric_date >= $daily_data_start_date
...
-- set date diff param
set num_days = (select datediff(day, min(POST_DATETIME), max(POST_DATETIME)) from _impact_score_per_influencer_by_post);
...
create or replace temporary table _influencer_impact_score_daily_derived as
with t1 as
    (select *,
           datediff(day, POST_DATETIME, cross_join_date) as days_between
           from _impact_score_per_influencer_by_post
    ...
    cross join (
          ...
          dateadd(
            ...
            dateadd(day, '+1', (select max(to_date(POST_DATETIME)) from _impact_score_per_influencer_by_post))
          ) as cross_join_date
    ...
    from t1
    left join _general_decay_curve as decay on decay.days_after_post = t1.days_between;



-- union the daily data with the derived post data
create or replace temporary table _influencer_data_daily as
select
     metric_date as date,
    ...
    hours_since_post,
    days_since_post
 from _impact_score_per_influencer_per_day
 where date >= $daily_data_start_date
...
select
       ...
       hours_since_post,
       days_since_post
from _influencer_impact_score_daily_derived
where  date < $daily_data_start_date;


-- create avg audience metrics table
create or replace temporary table _avg_audience_metrics as
select network,
...
from
    ...
    where publisherid in (select distinct publisher_id from _influencer_data_daily)
    order by PUBLISHERID, LASTUPDATED)
group by network;

-- create avg audience metrics by publisher
create or replace temporary table _avg_audience_metrics_by_publisher as
select PUBLISHERID,
...
create or replace temporary table _impact_score_with_modeled_metrics as
select
date,
publisher_id,
post_id,
       ...
       coalesce(female, female_fill, avg_female_by_publisher, avg_female_by_network, (select avg(avg_female_by_network) from _avg_audience_metrics)) as percent_female,
       coalesce(percent_us, percent_us_fill, avg_percent_us_by_publisher, avg_percent_us_by_network, (select avg(avg_percent_us_by_network) from _avg_audience_metrics)) as percent_us
...
from
    ...
    left join _avg_audience_metrics aam on lower(aam.network) = lower(i.social_network)
    left join _avg_audience_metrics_by_publisher aap on aap.PUBLISHERID = i.publisher_id
    order by i.publisher_id, i.date);



-- final modeled table: multiply impact score by percent female and percent us
create or replace transient table reporting_media_base_prod.public.influencer_impact_score_daily_sxna as
select date,
       publisher_id,
       publisher_name,
       post_id,
       ...
       engagements,
       impressions
       from _impact_score_with_modeled_metrics;
