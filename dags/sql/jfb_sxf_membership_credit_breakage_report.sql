CREATE OR REPLACE   TABLE SHOPIFY_TEST.PUBLIC.tmp_original_credit_activity AS
-- Gross VAT
SELECT CASE
...
CREATE OR REPLACE   TABLE SHOPIFY_TEST.PUBLIC.tmp_jfb_sxf_base AS
SELECT brand,
       region,
       country,
...
FROM (SELECT *
                   FROM SHOPIFY_TEST.PUBLIC.tmp_original_credit_activity C
         ...
         LEFT JOIN SHOPIFY_TEST.PUBLIC.tmp_original_credit_activity
...
WHERE original_credit_tender = 'Cash'
  AND 
  iff(brand in ('Fabletics Mens','Fabletics Womens','Yitty'),
...
CREATE OR REPLACE   TABLE SHOPIFY_TEST.PUBLIC.tmp_append_jfb_sxf_credits AS
SELECT a.brand || ' ' || a.country                                                         business_unit,
       ...
       datediff(MONTH, original_issued_month, '2025-07-01') + 1                             credit_tenure,
       '2025-07-01'                                                                         recognition_booking_month,
       ...
       iff((datediff(MONTH, original_issued_month, '2025-07-01') + 1) > 24, 0,
       ...
       iff(datediff(MONTH, original_issued_month, '2025-07-01') + 1 <= 24,
       ...
       CASE
           WHEN (datediff(MONTH, original_issued_month, '2025-07-01') + 1) > 24
               THEN
               CASE
...
FROM (SELECT *
                          FROM SHOPIFY_TEST.PUBLIC.tmp_jfb_sxf_base
                          WHERE original_credit_reason IN ('Membership Credit','Refund','Gift Card - Paid')
                            AND original_issued_month <= '2025-07-01'
                            AND activity_month <= '2025-07-01') a
         LEFT JOIN (SELECT brand,
                                   ...
                                   FROM (SELECT brand,
                                         ...
                                         FROM (SELECT brand,
                                               ...
                                               FROM SHOPIFY_TEST.PUBLIC.tmp_jfb_sxf_base
                                               WHERE activity_month BETWEEN DATEADD(MONTH, -23, '2025-07-01') AND '2025-07-01'
                                                 AND original_issued_month BETWEEN DATEADD(MONTH, -23, '2025-07-01') AND '2025-07-01'
                                                 AND currency = 'Local Gross VAT'
                                               GROUP BY brand, country, currency, original_credit_reason, credit_tenure))) r10
                                      ...
                                      FROM (SELECT brand,
                                            ...
                                            FROM (SELECT brand,
                                                  ...
                                                  WHERE activity_month BETWEEN DATEADD(MONTH, -23, '2025-07-01') AND '2025-07-01'
                                                    AND original_issued_month BETWEEN DATEADD(MONTH, -23, '2025-07-01') AND '2025-07-01'
                                                    AND currency = 'Local Gross VAT'
                                                  GROUP BY brand, country, currency, original_credit_reason, credit_tenure))) r24
                                 ...
                                 FROM (SELECT brand,
                                       ...
                                       FROM (SELECT brand,
                                             ...
                                             WHERE activity_month BETWEEN DATEADD(MONTH, -23, '2025-07-01') AND '2025-07-01'
                                               AND original_issued_month BETWEEN DATEADD(MONTH, -23, '2025-07-01') AND '2025-07-01'
                                               AND currency = 'Local Gross VAT'
                                             GROUP BY brand, country, currency, original_credit_reason, credit_tenure))) ur
...
GROUP BY a.brand,
         ...
         a.original_issued_amount,
         '2025-07-01',
         datediff(MONTH, original_issued_month, '2025-07-01') + 1,
         expected_redeemed_rate,
         expected_unredeemed_rate,
         expected_redeemed_rate_max
;

DELETE FROM SHOPIFY_TEST.PUBLIC.jfb_sxf_membership_credit_breakage_report
where recognition_booking_month = '2025-07-01';

INSERT INTO SHOPIFY_TEST.PUBLIC.jfb_sxf_membership_credit_breakage_report
SELECT business_unit,
       ...
       breakage_type
FROM SHOPIFY_TEST.PUBLIC.tmp_append_jfb_sxf_credits
;

DELETE FROM SHOPIFY_TEST.PUBLIC.jfb_sxf_membership_credit_breakage_report_snapshot
where recognition_booking_month = '2025-07-01';

CREATE TRANSIENT TABLE IF NOT EXISTS SHOPIFY_TEST.PUBLIC.jfb_sxf_membership_credit_breakage_report_snapshot (
...
INSERT INTO SHOPIFY_TEST.PUBLIC.jfb_sxf_membership_credit_breakage_report_snapshot
SELECT business_unit,
       brand,
       region,
       country,
...
FROM SHOPIFY_TEST.PUBLIC.jfb_sxf_membership_credit_breakage_report
;
