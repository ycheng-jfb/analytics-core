   ...
   COALESCE(LEAD(start_date) OVER (PARTITION BY associate_id ORDER BY start_date), CURRENT_TIMESTAMP) AS end_datetime
	...
	SELECT
	   ...
	   CASE WHEN DATEPART(MINUTE,CAST(o.DATETIME_ADDED  AT TIME ZONE 'Pacific Standard Time' AT TIME ZONE st.sf_timezone AS DATETIME)) < 30
		...
		COALESCE(nc.non_cash_credit_gross_of_vat_local_amount, 0) AS non_cash_credit_gross_of_vat_local_amount,
	...
	FROM ultramerchant.dbo."ORDER" o WITH(NOLOCK)
	    ...
	    LEFT JOIN (
	        SELECT oc.order_id,
            SUM(oc.amount) AS non_cash_credit_gross_of_vat_local_amount
        FROM #orders_base AS base
            JOIN ultramerchant.dbo.order_credit AS oc WITH(NOLOCK)
                ON oc.order_id = base.order_id
            LEFT JOIN ultramerchant.dbo.gift_certificate AS gc WITH(NOLOCK)
                ON gc.gift_certificate_id = oc.gift_certificate_id
            LEFT JOIN ultramerchant.dbo.gift_certificate_store_credit AS gcs WITH(NOLOCK)
                ON gcs.gift_certificate_id = gc.gift_certificate_id
            LEFT JOIN ultramerchant.dbo.store_credit AS sc WITH(NOLOCK)
                ON COALESCE(oc.store_credit_id, gcs.store_credit_id) = sc.store_credit_id
            LEFT JOIN ultramerchant.dbo.store_credit_reason AS scr WITH(NOLOCK)
                ON scr.store_credit_reason_id = sc.store_credit_reason_id
            LEFT JOIN ultramerchant.dbo.membership_token AS mt WITH(NOLOCK)
                ON mt.membership_token_id = oc.membership_token_id
            LEFT JOIN ultramerchant.dbo.membership_token_reason AS mtr WITH(NOLOCK)
        ...
        WHERE COALESCE(oc.store_credit_id, oc.membership_token_id, oc.gift_certificate_id) IS NOT NULL
            AND COALESCE(scr.cash, mtr.cash, 0) = 0 AND gc.order_id IS NULL
        GROUP BY oc.order_id
        ) nc ON nc.order_id = o.order_id
	...
	select o.*,
		...
		((COALESCE(revenue_gross_vat_local,0) - COALESCE(non_cash_credit_gross_of_vat_local_amount, 0)) /(1 + COALESCE(vrh.rate, 0))) * COALESCE(ocr.EXCHANGE_RATE, 1) as revenue_net_vat_usd
