...
(
    ...
    WHERE
        ...
        AND (order_status = 'Success' OR ((order_status = 'Pending' AND order_processing_status_code IN (2050,2060,2065))
        AND CAST(order_hq_datetime AS DATE) >= DATEADD(MONTH,-1, CURRENT_DATE()-1)))
    GROUP BY s.region
            ,o.product_sku
);
...
(
    ...
    WHERE order_line_status <>'Cancelled'
        AND (order_status = 'Success' OR ((order_status = 'Pending' AND order_processing_status_code IN (2050,2060,2065)) --includes Fulfillment (In Progress), FulFillment (Batching), Placed
        AND CAST(order_hq_datetime AS DATE) >= DATEADD(MONTH,-1, CURRENT_DATE()-1)))
...
TRUNCATE TABLE work.dbo.prebreak_size_scales;

INSERT INTO work.dbo.prebreak_size_scales
(
    month
    ,store_group
    ,sku
