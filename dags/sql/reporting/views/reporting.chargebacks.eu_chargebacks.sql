CREATE OR REPLACE VIEW reporting.chargebacks.eu_chargebacks AS
SELECT
...
FROM edw.data_model.fact_chargeback fc
    JOIN edw.stg.dim_customer c ON fc.customer_id = c.customer_id
    JOIN edw.stg.fact_order o ON fc.order_id = o.order_id
    JOIN lake_view.ultra_merchant.address a ON a.address_id = o.billing_address_id
    JOIN edw.stg.dim_store s ON s.store_id = fc.store_id
    JOIN lake_view.ultra_merchant."ORDER" umo ON o.order_id= umo.order_id
    JOIN edw.stg.dim_order_sales_channel dosc ON o.order_sales_channel_key=dosc.order_sales_channel_key
    JOIN edw.stg.dim_order_status os ON os.order_status_key = o.order_status_key
WHERE dosc.order_classification_l1='Product Order'  AND os.order_status = 'Success' AND s.store_region = 'EU'
    ...
    AND NOT EXISTS (SELECT 1 FROM lake.ultra_merchant.rma AS rma WHERE rma.order_id = o.order_id AND rma.statuscode IN (4670,4671))
