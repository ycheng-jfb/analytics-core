truncate table EDW_PROD.NEW_STG.fact_order_for_promotion;
...
insert into EDW_PROD.NEW_STG.fact_order_for_promotion
with p1 as (
with orders as (
   with metafield as (
    SELECT
...
where o.PROCESSED_AT >= '2025-10-27'
group by o.id
),
...
where ot.value = 'Test Order' and o.PROCESSED_AT >= '2025-10-27'
group by o.id
),
...
order_status_data as (
    ...
    from LAKE_MMOS.SHOPIFY_SHOEDAZZLE_PROD."ORDER" o
    left join LAKE_MMOS.SHOPIFY_SHOEDAZZLE_PROD.order_tag ot on o.id = ot.order_id
    where UPPER(ot.value) != 'TEST ORDER' or ot.value is null  and o.PROCESSED_AT >= '2025-10-27'
         ) t
    group by t.ID
),
...
left join t1 on t1.order_id = orders.order_id
where t1.order_id is null
and orders.order_local_datetime >='2025-10-27'
)

select  * from p1
where payment_transaction_local_datetime >= '2025-10-27'
union all
select * from p2
where payment_transaction_local_datetime >= '2025-10-27'
union all
