...
FROM (
    ...
    FROM (VALUES
        ...
        /* ULTRA_PARTNER */
        ('ultrapartner', 'fulfillment_partner_item_warehouse'),
        ('ultrapartner', 'fulfillment_partner_item_warehouse_log'),
        ('ultrapartner', 'fulfillment_partner_notification_log'),
        /* ULTRA_ROLLUP */
    ...
    SELECT
        sp.name,
        'ultrapartner' AS db_name,
        sa.name as table_name
    FROM ultrapartner.dbo.sysarticles AS sa with(nolock)
    JOIN ultrapartner.dbo.syspublications AS sp with(nolock)
    ON sa.pubid = sp.pubid
    WHERE sp.name LIKE '%HVR%'
        AND sa.status % 2 = 1
    UNION ALL
