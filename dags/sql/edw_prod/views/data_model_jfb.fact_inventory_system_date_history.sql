CREATE OR REPLACE VIEW data_model_jfb.fact_inventory_system_date_history
(
    item_id,
    warehouse_id,
...
SELECT
    ...
    available_to_sell_quantity,
    CASE
        WHEN date < '2023-04-10' THEN available_to_sell_quantity
        WHEN date >= '2023-04-10' AND date < '2024-02-14' THEN
            CASE
                WHEN warehouse_id = 231 THEN 0 -- Perris
                ELSE available_to_sell_quantity
            END
        WHEN date >= '2024-02-14' AND date < '2025-01-03' THEN
            CASE
                WHEN warehouse_id IN (231, 154, 107) THEN 0 -- Kentucky and Perris
                ELSE available_to_sell_quantity
            END
        WHEN date >= '2025-01-03' THEN
            CASE
    ...
    meta_create_datetime,
    meta_update_datetime
FROM stg.fact_inventory_system_date_history
