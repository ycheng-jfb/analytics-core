CREATE OR REPLACE PROCEDURE ACCOUNTING.MONTH_END.LOYALTY_POINT_FIXER_CONSOLIDATED()
...
LOOP_COMMAND = `
      ...
      FROM accounting.month_end.loyalty_points_balance
...
while (FIX_LOOP != -1) {
    ...
    LOOP_COMMAND = `
        UPDATE accounting.month_end.loyalty_points_balance AS b
        ...
        FROM (
            SELECT
                ...
                CASE WHEN A.fixed_balance < 0 AND A.points < 0 AND B.fixed_balance >= 0
                    ...
                    ELSE 0 END
                    AS fixed_points
            FROM accounting.month_end.loyalty_points_balance AS A
                LEFT JOIN accounting.month_end.loyalty_points_balance AS B
