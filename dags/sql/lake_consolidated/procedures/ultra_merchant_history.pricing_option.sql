CREATE or replace TRANSIENT TABLE lake_consolidated.ultra_merchant_history.PRICING_OPTION (
    data_source_id INT,
    meta_company_id INT,
...
SET lake_jfb_watermark = (
    ...
    FROM (
        ...
        FROM lake_jfb.ultra_merchant_history.PRICING_OPTION
    ) AS A
);


CREATE OR REPLACE TEMP TABLE _lake_jfb_pricing_option_history AS
SELECT DISTINCT
...
QUALIFY ROW_NUMBER() OVER(PARTITION BY pricing_option_id, META_SOURCE_CHANGE_DATETIME
    ORDER BY HVR_CHANGE_SEQUENCE DESC) = 1;


CREATE OR REPLACE TEMP TABLE _lake_jfb_company_pricing_option AS
SELECT pricing_option_id, company_id as meta_company_id
FROM (
         SELECT
            L.pricing_option_id,
            DS.company_id
           FROM lake_jfb.ultra_merchant_history.pricing_option AS L
            JOIN (
...
INSERT INTO lake_consolidated.ultra_merchant_history.PRICING_OPTION (
    data_source_id,
    meta_company_id,
    pricing_option_id,
    pricing_id,
    ...
    datetime_added,
    datetime_modified,
    meta_original_pricing_option_id,
    meta_row_source,
    hvr_change_op,
...
SELECT DISTINCT
    ...
    CASE WHEN NULLIF(s.pricing_option_id, '0') IS NOT NULL THEN CONCAT(s.pricing_option_id, s.meta_company_id) ELSE NULL END as pricing_option_id,
    CASE WHEN NULLIF(s.pricing_id, '0') IS NOT NULL THEN CONCAT(s.pricing_id, s.meta_company_id) ELSE NULL END as pricing_id,
    s.break_quantity,
    s.unit_price,
    s.shipping_price,
    s.datetime_added,
    s.datetime_modified,
    s.pricing_option_id as meta_original_pricing_option_id,
    s.meta_row_source,
    s.hvr_change_op,
...
FROM (
    ...
    FROM _lake_jfb_pricing_option_history AS A
    LEFT JOIN _lake_jfb_company_pricing_option AS CJ
        ON cj.pricing_option_id = a.pricing_option_id
    WHERE COALESCE(CJ.meta_company_id, 40) IN (10)
    ) AS s
WHERE NOT EXISTS (
    SELECT
        1
    FROM lake_consolidated.ultra_merchant_history.PRICING_OPTION AS t
    WHERE
        s.data_source_id = t.data_source_id
        AND  s.meta_company_id = t.meta_company_id
        AND          s.pricing_option_id = t.meta_original_pricing_option_id
        AND s.effective_start_datetime = t.effective_start_datetime
    )
ORDER BY s.effective_start_datetime ASC;


CREATE OR REPLACE temp table _pricing_option_updates as
SELECT s.*,
    row_number() over(partition by s.data_source_id,
s.meta_company_id,

        s.meta_original_pricing_option_id
...
FROM lake_consolidated.ultra_merchant_history.PRICING_OPTION as s
INNER JOIN (
    SELECT
        data_source_id,
meta_company_id,
        ...
        meta_original_pricing_option_id,
        effective_start_datetime
    FROM lake_consolidated.ultra_merchant_history.PRICING_OPTION
    WHERE effective_end_datetime is NULL
    ) AS t
    ON s.data_source_id = t.data_source_id
    AND s.meta_company_id = t.meta_company_id
    AND     s.meta_original_pricing_option_id = t.meta_original_pricing_option_id
WHERE (
...
CREATE OR REPLACE TEMP TABLE _pricing_option_delta AS
SELECT
    s.data_source_id,
s.meta_company_id,
...
FROM _pricing_option_updates AS s
    LEFT JOIN _pricing_option_updates AS t
    ON s.data_source_id = t.data_source_id
    AND s.meta_company_id = t.meta_company_id
    AND     s.meta_original_pricing_option_id = t.meta_original_pricing_option_id
    AND S.rnk = T.rnk - 1
...
UPDATE lake_consolidated.ultra_merchant_history.PRICING_OPTION AS t
SET t.effective_end_datetime = s.new_effective_end_datetime,
    META_UPDATE_DATETIME = current_timestamp()
FROM _pricing_option_delta AS s
