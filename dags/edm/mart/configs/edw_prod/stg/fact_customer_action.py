    ...
    return SnowflakeMartFactOperator(
        ...
        column_list=[
            ...
            Column(
                ...
                key_lookup_list=[
                    KeyLookupJoin(stg_column='customer_action_type', stg_column_type='VARCHAR(50)')
                ],
            ),
            Column('order_id', 'INT', foreign_key=ForeignKey('fact_order'), uniqueness=True),
            Column('meta_original_order_id', 'INT'),
            Column('store_id', 'INT', foreign_key=ForeignKey('dim_store')),
            Column('customer_action_local_datetime', 'TIMESTAMP_TZ(3)', uniqueness=True),
