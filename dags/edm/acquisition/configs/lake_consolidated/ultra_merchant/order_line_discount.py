...
table_config = TableConfig(
    table='order_line_discount',
    company_join_sql="""
        SELECT DISTINCT
            L.ORDER_LINE_DISCOUNT_ID,
            DS.company_id
        FROM {database}.REFERENCE.DIM_STORE AS DS
        INNER JOIN {database}.{schema}."ORDER" AS O
            ON DS.STORE_ID = O.STORE_ID
        INNER JOIN {database}.{source_schema}.order_line_discount AS L
            ON L.ORDER_ID = O.ORDER_ID""",
    column_list=[
        Column('order_line_discount_id', 'INT', uniqueness=True, key=True),
