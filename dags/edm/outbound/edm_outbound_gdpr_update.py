...
from include.airflow.operators.snowflake_load import (
    SnowflakeBrandIncrementalLoadOperator,
)
...
from task_configs.dag_config.gdpr_config import (
    GDPRLakeTargetConfig,
    PIIConfig,
    pii_config,
)
...
class PIIMaskOperator(BaseSnowflakeOperator):
    ...
    def full_table_name(self) -> str:
        full_table_name: str = GDPRLakeTargetConfig.FULL_TABLE_NAME.format(
            brand="consolidated"
        )
    ...
    def dml_mask_lake_cols(self) -> str:
        ...
        for conf in self.config:
            customer_id_column = (
                "meta_original_customer_id"
                if conf.use_meta_original_customer_id
                else "customer_id"
            )
    ...
    def mssql_exec_sp(self) -> None:
        ...
        with ConnClosing(self.snowflake_hook.get_conn()) as sf_cnx:
            ...
            request_job_ids_by_company_id = {
                i[0]: json.loads(i[1]) for i in sf_cursor.fetchall()
            }
        for company_id, request_job_ids in request_job_ids_by_company_id.items():
            ...
            with ConnClosing(
                self.mssql_hook(mssql_conn_id=mssql_conn_id).get_conn()
            ) as ms_cnx:
...
with dag:
    ...
    for brand, db_conn_map in brand_database_conn_mapping.items():
        full_table_name = GDPRLakeTargetConfig.FULL_TABLE_NAME.format(
            brand=brand.lower()
        )
        ...
        request_job_to_snowflake = SnowflakeBrandIncrementalLoadOperator(
            ...
            copy_config=CopyConfigCsv(
                header_rows=1, field_delimiter="\t", record_delimiter="\n"
            ),
            warehouse="DA_WH_ETL_LIGHT",
            initial_load=True,
        )
        (
            request_job_to_s3
            >> request_job_to_snowflake
            >> to_lake_consolidated
            >> pii_update
        )
