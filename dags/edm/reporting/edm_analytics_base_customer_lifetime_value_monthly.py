...
with dag:
    ...
    monthly_tasks_day2_branch_out = EmptyOperator(task_id='monthly_tasks_day2_branch_out')
    ...
    compare_fso_vs_cltv = SnowflakeAlertOperator(
        ...
        sql_or_path=Path(
            SQL_DIR, 'edw_prod', 'procedures', 'validation.finance_sales_ops_cltv_comparison.sql'
        ),
        database='edw_prod',
        subject="Alert: EDW - Comparison of metrics between FSO and CLTV",
        body=("Below is the list of differences between FSO and CLTV on cash gross revenue metric"),
        distribution_list=[
    ...
    chain_tasks(
        monthly_tasks_day2_to_day4_branch_out, check_edm_edw_load_execution, vip_tenure_final_output
    )
