...
def check_time(**context):
    ...
    if (execution_time.hour == 0 and execution_time.weekday() == 0) or (execution_time.hour == 5):
...
with dag:
    ...
    check_desired_schedule = BranchPythonOperator(task_id="check_time", python_callable=check_time)

    check_desired_schedule >> pcvr_session_historical >> pcvr_product_psource >> FL073_pcvr_psource
