...
class SSHHook(BaseHook):
    ...
    :type password: str
    :param key_file: path to key file to use to connect to the remote_host. ignored when private_key is given.
    :type key_file: str
    ...
    def __init__(
        ...
        if self.ssh_conn_id is not None:
            with TempLoggingWarning():
                print(f"getting conn {self.ssh_conn_id}")
            ...
            if conn.extra is not None:
                ...
                if not self.timeout and "timeout" in extra_options:
    ...
    def get_conn(self):
        ...
        if self.no_host_key_check:
            self.log.warning(
                "No Host Key Verification. This wont protect "
        ...
        if self.private_key:
            ...
            connect_kwargs.update(pkey=pkey)
        elif self.key_file:
    ...
    def create_tunnel(
        self, local_port: int, remote_port: int = None, remote_host: str = "localhost"
    ) -> SSHTunnelForwarder:
        """
