...
class FacebookAdsHook(BaseHook):
    def __init__(
        ...
        self.api_version = self.get_connection(self.facebook_ads_conn_id).extra_dejson.get(
            "api_version"
        )
    ...
    def access_token(self):
        if self._access_token is None:
            self._access_token = (
                self.connection.extra_dejson.get("access_token") or self.connection.password
            )
        ...
        while True:
            ...
            try:
                ...
                if try_count == max_tries:
                    ...
                    print(f"got exception {e}, waiting {wait_seconds} second before retry")
    ...
    def await_async_report(self, report_run_id, initial_wait=2, max_wait_time_seconds=60 * 60):
        ...
        while s.is_processing:
            ...
            print(f"job {report_run_id} has status '{s.status}'; waiting {wait_seconds} seconds")
            time.sleep(wait_seconds)
            s = self.get_async_report_status(report_run_id=report_run_id)
            if s.percent_complete == 0 and waiter.elapsed_time > 60 * max_minutes_before_progress:
                raise TimeoutError(
            ...
            if loops_since_change >= 5:
                raise TimeoutError(f"Processing stuck at {s.percent_complete} percent complete")

        if s.is_complete:
            return True
        else:
            raise ValueError(f"Unsuccesful job status returned ('{s.status}'). Failing job.")
    ...
    def get_rows(self, endpoint: str, payload: dict = None, max_retries_per_req=3):
        ...
        print(f"x-business-use-case-usage: {response.headers.get('x-business-use-case-usage')}")
    ...
    @staticmethod
    def get_rows_to_file(rows, filename, column_list, compression="gzip", add_timestamp=False):
    ...
    def get_rows_to_s3(self, rows, column_list, bucket, key, s3_replace=True, add_timestamp=False):
