...
class MsGraphMailHook(BaseHook):
    ...
    def get_conn(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            FileSystemTokenBackend(token_path=td, token_filename='tmp_token.txt').delete_token()
        return client
            ...
            for message in messages:
                if message.has_attachments and not message.is_read:
                    ...
                    for att in message.attachments:
                        ...
                        new_file_name = (
                            f"{file_parts[0]}{mail_time if add_timestamp else ''}.{file_parts[1]}"
                        )

                        if self.file_extensions is not None and file_parts[1].lower() in [
                            x.lower() for x in self.file_extensions
                        ]:
                            if self.attachments_list:
                                if file_name.lower() in [x.lower() for x in self.attachments_list]:
                                    att.save(location=file_path, file_name=new_file_name)
...
class MsGraphSharePointHook(BaseHook):
    ...
    @retry_wrapper(3, Exception, sleep_time=5)
    def list_file(self, site_id: str, drive_id: str, source_folder_name: str, file_path: str):
        """
