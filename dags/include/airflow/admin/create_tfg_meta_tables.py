...
def create_tfg_meta_tables(session=None):
    ...
    if hasattr(bind.dialect, 'has_schema') and not bind.dialect.has_schema(bind, 'tfg_meta'):
        print("creating schema")
        bind.execute(CreateSchema('tfg_meta'))
    ProcessState.metadata.create_all(bind, tables=[ProcessState.__table__, TaskState.__table__])
