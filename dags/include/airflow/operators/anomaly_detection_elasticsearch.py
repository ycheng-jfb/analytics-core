...
class BaseAnomalyDetectionOperator(BaseOperator):
    def __init__(
        self, elasticsearch_conn_id, kibana_conn_id, job_name, index_name, id_name, *args, **kwargs
    ):
    ...
    def get_job_last_timestamp(self):
        ...
        es_call = requests.get(
            post_url, auth=HTTPBasicAuth(self.es_hook.conn.login, self.es_hook.conn.password)
        )
        results_json = es_call.json()
        print(results_json)
        last_timestamp = results_json['jobs'][0]['data_counts']['latest_record_timestamp']
        left_off_date = str(
    ...
    def start_datafeed(self, start_time, end_time):
        ...
        post_ml_start_datafeed = (
            f'https://{self.es_hook.conn.host}/_ml/datafeeds/{self.datafeed_name}/_start'
        )
    ...
    def open_ml_job(self):
        ...
        post_ml_open_ml_job = (
            f'https://{self.es_hook.conn.host}/_ml/anomaly_detectors/{self.job_name}/_open'
        )
    ...
    def send_anomaly_email(
        self, anomalies_df, first_timestamp, last_timestamp, recipients=['snelson@techstyle.com']
    ):
    ...
    def execute(self, context):
        ...
        if len(anomaly_records_df) > 0:
            high_anomalies_df = self.get_high_anomalies(anomaly_records_df, anomaly_threshold=50)
            print('Anomalies: ', '\n', high_anomalies_df)
