...
from include.airflow.hooks.sprinklr import RateLimitException, SprinklrHook, TooMuchDataException
...
class SprinklrToS3Operator(BaseProcessWatermarkOperator):
    ...
    def from_unix(self):
        if self.from_date:
            try:
                return (
                    int(pendulum.parse(self.from_date).timestamp() * 1000) - self.look_back_period
                )

            except ParserError:
                print(f"Error parsing from_date '{self.from_date}', defaulting to low_watermark")
    ...
    def to_unix(self):
        if self.to_date:
            try:
                ...
                print(f"Error parsing to_date '{self.to_date}', defaulting to high_watermark")
    ...
    def get_rows(self, payload):
        ...
        while current_time < self.to_unix:
            ...
            ctime_str = str(pendulum.from_timestamp(current_time / 1000).replace(microsecond=0))
            to_time_str = str(pendulum.from_timestamp(to_time / 1000).replace(microsecond=0))
            ...
            while True:
                ...
                try:
                    response = self.sprinklr_hook.make_request("POST", url, payload=payload)
                ...
                for row in data['rows']:
                    row_of_dict = {heading: value for heading, value in zip(headings, row)}
                    row_of_dict['load_time'] = load_time
                    if self.extract_case_id:
                        case_id = int(re.search(r'^(\d+)', row_of_dict['CASE_TEXT'].strip())[1])
...
PAYLOADS = {
    'retention': {
        ...
        'projections': [
            {'heading': 'CASE_COUNT', 'measurementName': 'CASE_COUNT', 'aggregateFunction': 'SUM'},
            {
    ...
    'agent_response_sla': {
        ...
        'sorts': [{'heading': 'CASE_FIRST_RESPONSE_SLA_CUSTOMMETRIC_4446', 'order': 'DESC'}],
        'projectionDecorations': None,
        'additional': None,
        'skipResolve': False,
        'jsonResponse': False,
    ...
    'case_queue_sla': {
        ...
        'filters': None,
        'groupBys': [
            {'heading': 'CASE_DATA', 'dimensionName': 'CASE', 'groupType': 'FIELD', 'details': None}
        ],
        'projections': [
            {
        ...
        'sorts': [
            {'heading': 'SUM_CASE_QUEUE_SLA_AWAITING_RESPONSE_CUSTOMMETRIC_8344', 'order': 'DESC'}
        ],
        'projectionDecorations': None,
        'additional': None,
        'skipResolve': False,
    ...
    'agent_scorecard_process': {
        ...
        "groupBys": [
            ...
            },
            {
                ...
                "dimensionName": "UNIVERSAL_CASE_CUSTOM_PROPERTY",
                "groupType": "FIELD",
                "details": {"srcType": "CUSTOM", "fieldName": "5f2c08f98551567d14413c26"},
                "namedFilters": None,
            },
        ],
        "projections": [
    ...
    'agent_scorecard_response': {
        ...
        "sorts": [{"heading": "CASE_COUNT__SUM__CaseProcessingSLAReport", "order": "DESC"}],
        "projectionDecorations": None,
        "additional": None,
        "skipResolve": False,
        "jsonResponse": False,
