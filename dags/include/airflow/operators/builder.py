...
class BuilderABTestingToS3Operator(BaseRowsToS3CsvOperator, BuilderBaseOperator):
    ...
    def retrieve_variation_metadata(self, all_content):
        ...
        for single_builder_page in all_content['results']:
            ...
            if 'variations' in single_builder_page and (len(single_builder_page['variations']) > 0):
                ...
                single_page_results.update(self.get_builder_page_attributes(single_builder_page))

                single_page_results['control'] = {}
                single_page_results['control']['test_variation_name'] = single_builder_page['name']
                single_page_results['control']['test_variation_id'] = single_builder_page['id']
                ...
                for index, variation in enumerate(variations_list):
                    ...
                    single_page_results[f"Variant_{index + 1}"]['test_variation_name'] = (
                    ...
                    control_variation.append(single_page_results[f"Variant_{index + 1}"])
                    ...
                    unique_psources_string = "|".join(list(unique_psources))
                    single_page_results[f"Variant_{index + 1}"]['psources'] = unique_psources_string
                ...
                if (
                    'control' not in single_page_results['control']['test_variation_name'].lower()
        ...
        if (
            ...
            for block_element in single_builder_page['data']['blocks']:
                ...
                if (
                    ...
                    for tab_element in block_element['component']['options'].get('tabs', []):
                        for content_element in tab_element['content']:
                            if (
                                ...
                                and content_element['component']['name'] == '_Collection_'
                            ):
                                collection_element = self.get_psource_value(content_element)
                                # pdb.set_trace()
                                collections_list.append(collection_element)
    ...
    def get_psource_value(self, block_element):
        ...
        if (
            ...
            if 'gridAssetContainer' in block_element['component']['options']:
                collection_element['gridAssetContainer'] = block_element['component']['options'][
                    'gridAssetContainer'
                ]
            # # include gridUrlLink if it exists:
            if 'gridUrlLink' in block_element['component']['options']:
                collection_element['gridUrlLink'] = block_element['component']['options'][
                    'gridUrlLink'
                ]
    ...
    def get_builder_page_attributes(self, single_builder_page):
        ...
        if 'data' in single_builder_page and 'seoNoIndex' in single_builder_page['data']:
            builder_page_level_attributes['seoNoIndex'] = single_builder_page['data']['seoNoIndex']
        else:
        ...
        if 'previewUrl' in single_builder_page:
            builder_page_level_attributes['builder_page_url'] = single_builder_page['previewUrl']
        elif 'data' in single_builder_page and 'url' in single_builder_page['data']:
            builder_page_level_attributes['builder_page_url'] = single_builder_page['data']['url']
        else:
        ...
        builder_page_level_attributes['builder_page_name'] = single_builder_page.get('name')

        if 'published' in single_builder_page:
            builder_page_level_attributes['published'] = single_builder_page['published']
        (
        ...
        if 'firstPublished' in single_builder_page:
            ...
            ) = self.convert_unix_timestamp_to_date(single_builder_page['firstPublished'])
        else:
            builder_page_level_attributes['firstPublished_datetime_utc'] = ''
            builder_page_level_attributes['firstPublished_datetime_pst'] = ''
    ...
    def get_all_targeting(self, builder_metadata):
        ...
        for single_builder_page in builder_metadata['results']:
            ...
            single_page_targeting['targeting'] = self.parse_targeting(single_builder_page)
        ...
        for single_builder_page in ab_testing_components:
            ...
            if 'winningTest' in single_builder_page:
                ...
                if 'id' in single_builder_page['winningTest']:
                    called_test['winning_variation'] = single_builder_page['winningTest']['id']
    ...
    def BaseRowsToS3CsvOperator_execute(self):
        ...
        with tempfile.NamedTemporaryFile(mode='wb', delete=True) as temp_file:
            ...
            temp_file.flush()
            self.upload_to_s3(temp_file.name, bucket=self.bucket, key=self.all_components_key)
        ...
        with tempfile.NamedTemporaryFile(mode='wb', delete=True) as temp_file:
            ...
            temp_file.flush()
            self.upload_to_s3(
                temp_file.name, bucket=self.bucket, key=self.all_targeting_components_key
            )
    ...
    def unpack_list_of_dicts(original_list):
        ...
        for original_dict in original_list:
            ...
            non_list_values = {
                key: value for key, value in original_dict.items() if not isinstance(value, list)
            }

            # Loop through the list values
            list_values = [value for value in original_dict.values() if isinstance(value, list)]
