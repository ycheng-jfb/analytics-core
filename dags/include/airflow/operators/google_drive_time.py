...
class DrivingTime(BaseOperator):
    ...
    def process_google_api(self):
        ...
        for thread_number in range(1, self.number_of_threads + 1):
            ...
            end = total_records if thread_number == self.number_of_threads else start + record_slice
            threads = Thread(target=self.thread_loop, args=(fl_vip_zip, start, end, thread_number))
            threads.start()
