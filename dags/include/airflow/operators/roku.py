...
class RokuToS3Operator(BaseOperator):
    ...
    def execute(self, context=None):
        ...
        self.report_params['reports'][0]['start_at'] = start_time.strftime('%Y-%m-%dT%H:%M:%SZ')
        self.report_params['reports'][0]['end_at'] = end_time.strftime('%Y-%m-%dT%H:%M:%SZ')
        self.report_params['reports'][0]['delivery_address'] = url
        response = hook.make_request(method="POST", endpoint="reports", json=self.report_params)
        print(response.json())
        data = response.json()
        ...
        while True:
            ...
            status = hook.make_request(method="GET", endpoint="reports", params={"uid": uid})
