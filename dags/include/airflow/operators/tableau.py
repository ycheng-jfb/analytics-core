...
class TableauRefreshOperator(BaseOperator):
    ...
    def validate_args(self):
        if self._data_source_id is not None and self.data_source_name is not None:
            raise ValueError(
                "only one of data_source_id and data_source_id may be given"
            )
...
class BaseTableauToS3Operator(BaseRowsToS3CsvOperator):
    ...
    def tableau_hook(self):
        hook = TableauHook(
            tableau_conn_id=self.tableau_conn_id, version=self.tableau_api_version
        )
        return hook
...
class TableauDeleteUsers(BaseOperator):
    ...
    def execute(self, context=None):
        all_users_gen = self.tableau_hook.api_get_call(
            api_path="users", api_object_name="users"
        )
