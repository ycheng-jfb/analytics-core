...
class AdyenToS3Operator(BaseSnowflakeOperator):
    ...
    def get_journal_type_from_mutations(df, journal_type, column_rename_map={}):
        ...
        return df_j.drop(['Register', 'Journal Type'], axis=1).rename(column_rename_map, axis=1)

    def transform_report(self, input_path):
        # Get dataframes needed for desired metrics
        df_mu = pd.read_excel(input_path, sheet_name='Mutations', header=4, usecols='A:C,F,H')
        df_pm = pd.read_excel(
            input_path, sheet_name='Payment Method Breakdown', header=4, usecols='B:C,G,I'
        )

        # Chargeback
        df_cb = self.get_journal_type_from_mutations(
            df_mu, 'Chargeback', column_rename_map={'Count': 'cb_count', 'Amount': 'cb_amount'}
        )
        ...
        df_cb_rev = self.get_journal_type_from_mutations(
            df_mu,
            'ChargebackReversed',
            column_rename_map={'Count': 'cb_reversed_count', 'Amount': 'cb_reversed_amount'},
        )
        ...
        df_second_cb = self.get_journal_type_from_mutations(
            df_mu,
            'SecondChargeback',
            column_rename_map={'Count': 'second_cb_count', 'Amount': 'second_cb_amount'},
        )
        ...
        df_final = reduce(lambda x, y: pd.merge(x, y, how='outer', on='Merchant Code'), dfs)
