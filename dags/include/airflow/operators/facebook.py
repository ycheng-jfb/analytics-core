...
class FacebookAdvancedAnalyticsToS3OperatorNew(FacebookAdvancedAnalyticsToS3Operator):
    ...
    def get_macros(self, account_ids):
        ...
        for store, listid, account_id in account_ids:
            result["LISTID_ALL_ACCOUNTS"] = (
                result["LISTID_ALL_ACCOUNTS"] + "," + str(account_id)
            )
        ...
        if result["LISTID_ALL_ACCOUNTS"][0] == ",":
            result["LISTID_ALL_ACCOUNTS"] = result["LISTID_ALL_ACCOUNTS"].replace(
                ",", "", 1
            )
        return result
    ...
    def get_rows(self):
        ...
        for idx, row in enumerate(rows):
            ...
            if response.status_code == 200:
                ...
                row_data["effective_object_story_id"] = data.get(
                    "effective_object_story_id"
                )
                row_data["object_story_spec"] = data.get("object_story_spec")
                row_data["instagram_permalink_url"] = data.get(
...
class FacebookAdInsightsToS3(BaseRowsToS3CsvOperator):
    ...
    def get_rows(self) -> Iterable[Dict]:
        ...
        if self.get_status(response):
            ...
            print(
                f"Generated the report for {self.account_id} with the run id: {report_run_id}"
            )
            ...
            res = requests.get(
                url=f"{report_run_url}/insights",
                params=access_token | {"limit": self.rows_per_req},
            )
                ...
                while True:
                    ...
                    if data["data"]:
                        for rec in data["data"]:
                            rec["api_call_timestamp"] = (
                                pendulum.DateTime.utcnow().isoformat()
                            )
                            yield rec
