...
class AlchemerBase(BaseRowsToS3CsvWatermarkOperator):
    ...
    def alchemer_hook(self):
        return AlchemerHook(conn_id=self.hook_conn_id) if self.hook_conn_id else AlchemerHook()
...
class AlchemerGetSurveyResponses(AlchemerBase):
    ...
    def get_rows(self):
        ...
        for survey in surveys:
            ...
            if (
                ...
                and parse(survey['modified_on']).timestamp() > parse(self.low_watermark).timestamp()
            ):
                responses = self.alchemer_hook.make_get_request_all(
                    f'survey/{survey["id"]}/surveyresponse', params=self.get_status_filter_param()
                )
            else:
                responses = self.alchemer_hook.make_get_request_all(
                    f'survey/{survey["id"]}/surveyresponse',
                    params={**self.get_submitted_filter_params(), **self.get_status_filter_param()},
                )
            for resp in responses:
                try:
                    # if no response data, continue to next response
                    if type(resp['survey_data']) == list and len(resp['survey_data']) == 0:
                        continue
                    ...
                    if isinstance(resp['url_variables'], dict):
                        if resp['url_variables'].get('customer_id'):
                            resp_dict['customer_id'] = resp['url_variables']['customer_id'].get(
                                'value'
                            )
                    ...
                    for metadata_name in metadata_names:
                        ...
                        yield {**resp_dict, **field_dict, **field_details, **updated_at_d}
                    ...
                    for ques_id, ques_resp in resp['survey_data'].items():
                        ...
                        if ques_resp.get('parent'):
                        ...
                        elif str(ques_resp['type']).upper() == 'RANK' and ques_resp.get('answer'):
                            ...
                            resp_details = self.options_resp_details(
                                survey_questions, ques_resp, ques_details, survey, ques_id_padded
                            )
                        else:
                            resp_details = [
                                self.format_details(
                    ...
                    print(f"Error - Survey ID: {survey['id']} | Response ID: {resp['id']}")
    ...
    def format_details(
        ...
        if l2_question_label:
            b['question_label'] = f'{l1_question_label} | {l2_question_label} | {l3_question_label}'
        ...
        for sub_ques_id, sub_ques_resp in ques_resp['subquestions'].items():
            if (
                ...
                for key in sub_ques_resp.keys():
                    ...
                    split_labels = self.clean_text(sub_sub_question.get('question')).split(' : ')
