...
dag = DAG(
    'membership_event',   # DAG任务名   建议python 脚本名字，DAG任务名，sql脚本名三者统一
...
run_fact_membership_event = SnowflakeOperator(
    task_id='fact_membership_event',
    sql='sql/fact_membership_event.sql',  # 执行脚本的相对路径
    snowflake_conn_id='snowflake_default',     # 配置的snowflake连接地址名字
    dag=dag,
)


run_fact_registration = SnowflakeOperator(
    task_id='fact_registration',
    sql='sql/fact_registration.sql',  # 执行脚本的相对路径
    snowflake_conn_id='snowflake_default',     # 配置的snowflake连接地址名字
    dag=dag,
)


run_fact_activation = SnowflakeOperator(
    task_id='fact_activation',
    sql='sql/fact_activation.sql',  # 执行脚本的相对路径
    snowflake_conn_id='snowflake_default',     # 配置的snowflake连接地址名字
    dag=dag,
)
